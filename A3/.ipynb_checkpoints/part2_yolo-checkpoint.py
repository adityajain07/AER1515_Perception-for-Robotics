...
# OPTIONS
###########################################################
image_path = 'data/test/left/000015.png'
path_split = image_path.split('/')
image_name = path_split[-1]
image_split = image_name.split('.')
image_num   = image_split[0]
yolo_dir = 'yolo'
write_dir = './data/test/est_bboxes/'
...
# bounding box coordinates to export
bbox_dict = {}
bbox_cnt  = 0
...
if len(idxs) > 0:
    ...
    for i in idxs.flatten():
        ...
        if LABELS[classIDs[i]]=='car':
            bbox_dict[bbox_cnt] = {}
            bbox_dict[bbox_cnt][0] = x
            bbox_dict[bbox_cnt][1] = y
            bbox_dict[bbox_cnt][2] = w
            bbox_dict[bbox_cnt][3] = h
            bbox_cnt += 1

# show the output image
cv2.imwrite(write_dir + image_name, image)
with open(write_dir + image_num + '.json', "w") as outfile:
    json.dump(bbox_dict, outfile, indent=4)
